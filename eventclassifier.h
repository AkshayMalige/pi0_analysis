...
Int_t getMultBin() {
    ...
    Int_t mult_bin = 5;
    if (mult_meta > 39 && mult_meta <= 56)  mult_bin = 4; // most peripheral
    if (mult_meta > 56 && mult_meta <= 77)  mult_bin = 3;
    if (mult_meta > 77 && mult_meta <= 102)  mult_bin = 2;
    if (mult_meta > 102 && mult_meta <= 169) mult_bin = 1; // most central
    if (mult_meta > 169) mult_bin = 0;

    return mult_bin;
    }

Int_t getMultImpactBin() {
    ...
    Float_t imp = fSubHeader->getImpactParameter();

    Int_t mult_bin = 5;
    if (imp >  0 && imp <= 1) mult_bin = 4; // most peripheral
    if (imp >  1 && imp <= 2) mult_bin = 3;
    if (imp >  2 && imp <= 3) mult_bin = 2;
    if (imp >  3 && imp <= 4) mult_bin = 1; // most central
    if (imp >  4) mult_bin = 0;
...
Int_t eventClassifierMultImpRPhiTarget() {
    Int_t mult_bin   = getMultImpactBin();
    Int_t target_bin = getTargetBin();
    Int_t rphi_bin   = getRPhiBin();

    return 300*mult_bin + 16*rphi_bin + target_bin;
}
Int_t eventClassifierRPhiTarget() {
    Int_t target_bin = getTargetBin();
    Int_t rphi_bin   = getRPhiBin();

    return 16*rphi_bin + target_bin;
}
