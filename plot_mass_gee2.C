...
void plot_mass_gee2(TString inFile, TString outputFile, Int_t nev=1000., Bool_t useMix=kFALSE)
    ...
    //void plot_mass_gee(TString inFile, TString outputFile, Int_t nev=100000000., Bool_t useMix=kFALSE)
{
    ...
    TFile *out    = new TFile("diphoton_massEXP_gen2_opAng5_C10_50.root","RECREATE");
    out->cd();
    ...
    TH2F* hAll_GEPmassAngGEP = new TH2F("hAll_GEPmassAngGEP",";M_{#gamma e^{+}e^{-}} [MeV/c^{2}];#alpha _#gamma{e^{+}e^{-}}",1000,0,1000.0,120,0,120);
    ...
    Int_t ptbins = 10.; // for pi0 reduced
    Int_t ptmin  = 0;
    Int_t ptmax  = 1000;
   ...
   TH1F*  MassPtY_all[ptbins][ybins] ;
	...
	if(pid.sizeG !=2)                        continue;  // still only cluster size 1 are calibrated, higer size are not calibrated
...
//	if(pid.angleGEP< 5 || pid.angleGEP> 60 ) continue;
	...
	hAll_GEPmassAngGEP->Fill(pid.massGEP,pid.angleGEP);
        //  hAll_GEPmassMultmulttofrpc->Fill(pid.massGEP,pid.mult_tofrpc);
